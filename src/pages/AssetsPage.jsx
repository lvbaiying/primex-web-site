...
const AssetsPage = ({ t }) => {
  const categories = [
    {
      title: t.assets.categories.private_credit.title,
      yield: t.assets.categories.private_credit.yield,
      risk: t.assets.categories.private_credit.risk,
      icon: (
        <CircleDollarSign
          size={64}
          strokeWidth={0.75}
    ...
    {
      title: t.assets.categories.real_estate.title,
      yield: t.assets.categories.real_estate.yield,
      risk: t.assets.categories.real_estate.risk,
      icon: (
        <Building2
          size={64}
          strokeWidth={0.75}
    ...
    {
      title: t.assets.categories.pre_ipo.title,
      yield: t.assets.categories.pre_ipo.yield,
      risk: t.assets.categories.pre_ipo.risk,
      icon: (
        <TrendingUp
          size={64}
          strokeWidth={0.75}
                  ...
                  <div className='flex gap-12 border-t border-zinc-100 pt-8'>
                    <div>
                      <div className='text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1'>
                        {t.assets.labels.target_yield}
                      </div>
                      <div className='text-lg font-mono'>{cat.yield}</div>
                    </div>
                    <div>
                      <div className='text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1'>
                        {t.assets.labels.risk}
                      </div>
      ...
      <div className='py-32 bg-zinc-50 border-y border-zinc-200'>
        <div className='container mx-auto px-6 md:px-12'>
          <div className='grid lg:grid-cols-2 gap-20'>
            <div>
              <SectionHeading
                title={t.assets.criteria_title}
                subtitle={t.assets.criteria.subtitle}
              />
            ...
            <div className='grid gap-6'>
              {[
                {
                  label: t.assets.criteria.hard_criteria.label,
                  items: t.assets.criteria.hard_criteria.items,
                },
                {
                  label: t.assets.criteria.optional.label,
                  items: t.assets.criteria.optional.items,
                },
              ].map((grp, i) => (
                <div
                  key={i}
