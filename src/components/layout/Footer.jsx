        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-12 mb-20">
          {links.map((section) => (
            <div key={section.title}>
              <h5 className="font-bold mb-8 uppercase text-[10px] tracking-[0.2em] text-zinc-400">{section.title}</h5>
              <ul className="space-y-4 text-sm font-medium">
                {section.items.map(item => (
                  <li key={item}><a href="#" className="hover:text-zinc-500 transition-colors">{item}</a></li>
                ))}
              </ul>
            </div>
          ))}
