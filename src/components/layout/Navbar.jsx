  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${
        scrolled
          ? 'bg-white/95 backdrop-blur-md border-b border-zinc-100 py-4'
          : 'bg-transparent py-8'
      }`}
    >
      <div className='container mx-auto px-6 md:px-12 flex justify-between items-center'>
        <div
          className='flex items-center gap-3 text-2xl font-bold tracking-tighter cursor-pointer'
        ...
        <div className='hidden lg:flex items-center gap-6'>
          <button
            onClick={toggleLang}
            className='flex items-center gap-1 text-sm font-medium text-zinc-500 hover:text-black transition-colors'
          >
            <Globe size={16} /> {lang === 'en' ? 'EN' : '繁'}
          </button>
          <Button
            variant='primary'
        ...
        <div className='lg:hidden absolute top-full left-0 w-full bg-white border-b border-zinc-100 p-8 flex flex-col gap-6 shadow-xl h-screen'>
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => handleNav(item.id)}
              className='text-2xl font-light text-left hover:pl-4 transition-all'
            >
              {item.label}
            </button>
          ))}
