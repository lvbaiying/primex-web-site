    ...
    <>
      ...
      >
        <div className='container mx-auto px-6 md:px-12 flex justify-between items-center'>
          <div
            className='flex items-center gap-2 text-2xl font-bold tracking-tighter cursor-pointer z-50'
            onClick={() => handleNav('home')}
          >
            <div className='w-8 h-8 bg-black text-white flex items-center justify-center rounded-sm shadow-xl'>
              P
            </div>
          ...
          <nav className='hidden lg:flex items-center gap-10'>
            {navItems.map((item) => (
              <button
                ...
                className={`text-xs uppercase tracking-[0.15em] font-bold relative group ${
                  activeTab === item.id
                    ? 'text-black'
